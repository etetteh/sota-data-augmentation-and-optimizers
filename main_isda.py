...
if args.adamod:
    ...
    optimizer = AdaMod([{'params': model.parameters()},
                        {'params': fc.parameters()}], lr=args.learning_rate, weight_decay=args.weight_decay)
...
if args.adalook:
    ...
    optimizer = Lookahead(AdaMod([{'params': model.parameters()},
                                  {'params': fc.parameters()}], lr=args.learning_rate, weight_decay=args.weight_decay))
                          
if args.ranger:   
    print("\n Using RAdam+LookAhead optimizer")
    optimizer = Lookahead(RAdam([{'params': model.parameters()},
                                 {'params': fc.parameters()}], lr=args.learning_rate, weight_decay=args.weight_decay))
...
if args.resume:
    checkpoint = torch.load('./checkpoint/Baseline_'+args.path+'_isda_ckpt.pth')
...
def test(test_loader, model, fc, criterion, epoch):
...
for epoch in range(start_epoch, args.epochs):
    ...
    top1, top5, losses = test(test_loader, model, fc, ce_criterion, epoch)
    ...
    if top1.avg > best_top1:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/Baseline_'+args.path+'_isda_ckpt.pth')
